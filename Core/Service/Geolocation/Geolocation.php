...
define ('SxGeoCity',  'Core' . DIRECTORY_SEPARATOR . 'Service' . DIRECTORY_SEPARATOR . 'Geolocation' .DIRECTORY_SEPARATOR . 'GeoCity.dat');
...
class Geolocation
{
     ...
     * @return int
     */
    private function search_idx($ipn, $min, $max): int
    {
        if($this->batch_mode){
            while($max - $min > 8){
                $offset = ($min + $max) >> 1;
     ...
     * @return array|bool
     */
    protected function parseCity(int $seek, $full = false)
    {
        if(!$this->pack) {
            return false;
        }
     ...
     * @return array
     */
    protected function unpack($pack, string $item = ''): array
    {
        $unpacked = array();
        $empty = empty($item);
        $pack = explode('/', $pack);
